...
| **タイル表示** | 2〜4 個のタブをコンテンツ領域に同時並列表示（左右・3分割・2×2） |
| **ドラッグで埋め込み** | 別アプリのタイトルバーをドラッグしてタブバーへドロップするだけで管理に追加 |
...
タブバー上で `Ctrl+クリック` により 2〜4 個のタブを複数選択し、コンテキストメニューまたはコマンドパレットから **タイル表示** を実行すると、選択したタブが同一のコンテンツ領域に並列表示されます。

- 2 タブ: 左右均等
- 3 タブ: 左半分 ＋ 右上 ＋ 右下
- 4 タブ: 2×2 グリッド

タイルを解除するにはコマンドパレット（`Alt+P`）から **タイル解除** を実行します。

### ドラッグ＆ドロップで埋め込む

別のアプリのタイトルバーをドラッグして Windowz のタブバーの上に重ねると、ドロップゾーンのハイライトが表示されます。そのままドロップするとそのウィンドウが自動的にタブとして追加されます。

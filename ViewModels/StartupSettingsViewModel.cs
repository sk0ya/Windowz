...
public partial class TileSetItem : ObservableObject
{
    ...
    [ObservableProperty]
    private ObservableCollection<StartupApplicationSetting> _apps = new();
...
public partial class StartupSettingsViewModel : ObservableObject
{
    ...
    public ObservableCollection<StartupApplicationSetting> GetAvailableAppsForTileSet()
    {
        var usedApps = TileSets.SelectMany(ts => ts.Apps).ToHashSet();
        var available = new ObservableCollection<StartupApplicationSetting>();
    ...
    [RelayCommand]
    private void RemoveAppFromTileSet(StartupApplicationSetting app)
    {
    ...
    [RelayCommand]
    private void MoveAppUp(StartupApplicationSetting app)
    {
        var tileSet = TileSets.FirstOrDefault(ts => ts.Apps.Contains(app));
        if (tileSet == null) return;
    ...
    [RelayCommand]
    private void MoveAppDown(StartupApplicationSetting app)
    {
        var tileSet = TileSets.FirstOrDefault(ts => ts.Apps.Contains(app));
        if (tileSet == null) return;
